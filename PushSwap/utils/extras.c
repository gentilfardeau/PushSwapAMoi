...
{
	int i;
	char *content_array[8] = {"content", "stacked", "above_median", "cost", "target_pos", "current_pos", "previous", "next"};
	...
	while(stack)
	{
		...
		if (i == 4)
			ft_printf("Target pos of the node : %d \n", stack->target_pos);
		if (i == 5)
